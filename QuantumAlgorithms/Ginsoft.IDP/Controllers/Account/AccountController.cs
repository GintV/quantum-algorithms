    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> ExternalLoginCallback()
        {
            ...
            if (user == null)
            {
                ...
                // email claim from Facebook
                var email = claims.FirstOrDefault(c => c.Type == "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress")?.Value ?? Guid.NewGuid().ToString();

                var returnUrlAfterRegistration = Url.Action("ExternalLoginCallback", new { returnUrl = returnUrl });
                var continueWithUrl = Url.Action("RegisterUser", "UserRegistration", new { returnUrl = returnUrlAfterRegistration, provider = provider, providerUserId = userId, email = email });
